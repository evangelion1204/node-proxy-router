...
import Updater from '../../src/updater/eskip'

chai.use(sinonChai)

describe.skip('eskip-Updater', function() {
    let mockedRouter
    ...
    it('read should process the content of a file', function (done) {
        ...
        updater.read(__dirname + '/../stubs/delta-routes.eskip', function (err) {
            expect(updater.process).to.be.calledWith(['route2: Path("/") -> "http://domain2.tld";'], ['route1'])
            done()
        })
    })
})